...
class TranscriptionServer:
    ...
    def run(self,
        ...
        with serve(
            ...
            host,
            port,
            ping_interval=None,      #  disable pings (safe if Node closes cleanly)
            close_timeout=3,         # shorter close timeout
            max_size=5 * 1024 * 1024 # allow up to 5MB per message (Twilio buffers)
        ) as server:
